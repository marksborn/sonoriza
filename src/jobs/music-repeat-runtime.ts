 ...
 * became stale while collection/planning was in progress. Because the writer
 * is created only after collectIncrementally returns, a rejection here causes
 * zero Spotify playlist writes.
 */
export async function revalidateMusicRepeatBeforeRealWrite(
  plan: PlanRunResult,
): Promise<void> {
  const state = currentMusicRepeatState();
  if (!state || state.simulate || !state.context.enabled) return;
