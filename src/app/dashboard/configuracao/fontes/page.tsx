...
async function addSource(formData: FormData) {
  ...
  try {
    ...
    if (spotifyType === SpotifySourceType.PLAYLIST) {
      ...
      if (!scopeIncludes(spotifyAccount.scope, LIBRARY_SCOPE)) {
...
export default async function SpotifySourcesPage({
  ...
  const availablePlaylists = playlists.filter(
    (playlist) => !configuredKeys.has(sourceKey(SpotifySourceType.PLAYLIST, playlist.id)),
  );
      ...
      <div className="relative mx-auto max-w-6xl">
        <header className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-violet-200/75 sm:text-base">
              Escolha playlists para músicas ou podcasts e programas salvos para episódios.
              O Sonoriza valida cada fonte diretamente na sua conta antes de salvar.
            </p>
          </div>
        ...
        {params.error && (
          <div className="mt-7 rounded-2xl border border-orange-400/25 bg-orange-400/10 px-4 py-3 text-sm font-bold text-orange-200">
            {params.error === "scope"
              ? "A conta Spotify precisa ser reconectada para liberar a leitura dos programas salvos."
        ...
        {!spotifyAccount ? (
          <section className="mt-7 rounded-[1.75rem] border border-orange-400/20 bg-[linear-gradient(145deg,rgba(62,17,116,0.96),rgba(30,8,66,0.96))] p-6 shadow-[0_24px_70px_-40px_rgba(255,107,0,0.55)] sm:p-8">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-violet-200/70">
              A configuração de fontes usa somente itens visíveis na sua própria conta e
              não aceita IDs digitados manualmente.
            </p>
            <form action={connectSpotify}>
              <button
                type="submit"
                className="mt-5 rounded-xl bg-gradient-to-r from-[#ff6b00] to-[#ff8a00] px-5 py-3 text-sm font-black text-white transition hover:brightness-110"
              >
                Conectar Spotify
              </button>
            </form>
          ...
          <>
            {!hasLibraryScope && (
              <section className="mt-7 flex flex-col gap-4 rounded-2xl border border-orange-400/25 bg-orange-400/10 p-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="font-black text-orange-200">Libere também os programas salvos</p>
                  <p className="mt-1 max-w-3xl text-sm leading-6 text-orange-100/70">
                    Sua conexão atual já permite listar playlists. Para mostrar seus programas
                    de podcast, o Spotify precisa autorizar uma vez o acesso de leitura à biblioteca.
                  </p>
                </div>
                <form action={connectSpotify}>
                  <button
                    type="submit"
                    className="whitespace-nowrap rounded-xl border border-orange-300/35 bg-orange-300/10 px-4 py-2.5 text-sm font-black text-orange-100 transition hover:bg-orange-300/20"
                  >
                    Reconectar Spotify
                  </button>
                </form>
              ...
              {configuredSources.length === 0 ? (
                <div className="mt-5 rounded-2xl border border-dashed border-violet-400/25 bg-violet-950/30 p-6 text-center">
                  ...
                  <p className="mt-1 text-sm text-violet-200/60">
                    Adicione uma playlist ou programa nas listas abaixo.
                  </p>
                </div>
              ) : (
                    ...
                    <article
                      ...
                      className={`rounded-2xl border p-4 ${
                        source.enabled
                          ...
                          : "border-violet-500/15 bg-violet-950/25 opacity-70"
                      }`}
                    >
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="rounded-full border border-violet-300/20 bg-violet-400/10 px-2.5 py-1 text-[0.68rem] font-black uppercase tracking-wide text-violet-200">
                              {source.spotifyType === SpotifySourceType.SHOW ? "Programa" : "Playlist"}
                            </span>
                          ...
                          <h3 className="mt-3 truncate font-black text-white">
                            {source.name ?? source.spotifyId}
                          </h3>
                          <p className="mt-1 truncate text-xs text-violet-200/45">
                            {source.spotifyId}
                          </p>
                        </div>
                        <span
                          className={`h-2.5 w-2.5 shrink-0 rounded-full ${
                            source.enabled ? "bg-emerald-300" : "bg-violet-500"
                          }`}
                          title={source.enabled ? "Ativa" : "Desativada"}
                        />
                      </div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <form action={toggleSource}>
                          ...
                          <button
                            type="submit"
                            className="rounded-xl border border-violet-300/25 bg-violet-500/10 px-3 py-2 text-xs font-black text-violet-100 transition hover:bg-violet-500/20"
                          >
                        ...
                        <form action={removeSource}>
                          ...
                          <button
                            type="submit"
                            className="rounded-xl border border-red-300/20 bg-red-400/10 px-3 py-2 text-xs font-black text-red-200 transition hover:bg-red-400/20"
                          >
                            Remover
                          </button>
                        </form>
            ...
            <div className="mt-5 grid gap-5 lg:grid-cols-2">
              <section className="rounded-[1.75rem] border border-violet-400/20 bg-[linear-gradient(145deg,rgba(42,15,94,0.92),rgba(22,6,53,0.94))] p-5 sm:p-6">
                ...
                <p className="mt-1 text-sm leading-6 text-violet-200/65">
                  Escolha se a playlist fornece músicas ou episódios de podcast.
                </p>

                {playlistLoadError ? (
                  <div className="mt-5 rounded-2xl border border-orange-400/20 bg-orange-400/10 p-4 text-sm text-orange-200">
                    Não foi possível listar suas playlists agora.
                  </div>
                ) : availablePlaylists.length === 0 ? (
                  <div className="mt-5 rounded-2xl border border-dashed border-violet-400/20 bg-violet-950/30 p-5 text-sm text-violet-200/60">
                  ...
                  <div className="mt-5 max-h-[38rem] space-y-3 overflow-y-auto pr-1">
                    {availablePlaylists.map((playlist) => (
                      <form
                        key={playlist.id}
                        action={addSource}
                        className="rounded-2xl border border-violet-400/20 bg-violet-950/35 p-4"
                      >
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 truncate text-xs text-violet-200/50">
                              {playlist.ownerName ?? "Spotify"}
                              {playlist.collaborative ? " · colaborativa" : ""}
                            </p>
                          </div>
                        </div>
                        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                          <select
                            name="kind"
                            defaultValue={SourceKind.MUSIC}
                            className="min-w-0 flex-1 rounded-xl border border-violet-300/25 bg-[#160638] px-3 py-2 text-sm font-bold text-violet-100 outline-none focus:border-violet-300/60"
                          >
                          ...
                          <button
                            type="submit"
                            className="rounded-xl bg-gradient-to-r from-[#ff6b00] to-[#ff8a00] px-4 py-2 text-sm font-black text-white transition hover:brightness-110"
                          >
                            Adicionar
                          </button>
                        </div>
              ...
              <section className="rounded-[1.75rem] border border-violet-400/20 bg-[linear-gradient(145deg,rgba(42,15,94,0.92),rgba(22,6,53,0.94))] p-5 sm:p-6">
                ...
                <h2 className="mt-1 text-xl font-black">Adicionar programa</h2>
                <p className="mt-1 text-sm leading-6 text-violet-200/65">
                  Programas salvos entram automaticamente como fonte de podcast.
                </p>
                ...
                {!hasLibraryScope ? (
                  <div className="mt-5 rounded-2xl border border-orange-400/20 bg-orange-400/10 p-5">
                    ...
                    <p className="mt-2 text-sm leading-6 text-orange-100/70">
                      Reconecte o Spotify acima para o Sonoriza poder ler a lista de programas salvos.
                    </p>
                  </div>
                ) : showLoadError ? (
                  <div className="mt-5 rounded-2xl border border-orange-400/20 bg-orange-400/10 p-4 text-sm text-orange-200">
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 truncate text-xs text-violet-200/50">
                              {show.publisher ?? "Programa do Spotify"}
                            </p>
                          </div>
                          <button
                            type="submit"
                            className="shrink-0 rounded-xl border border-violet-300/25 bg-violet-500/10 px-4 py-2 text-sm font-black text-violet-100 transition hover:bg-violet-500/20"
                          >
                            Adicionar
                          </button>
                        </div>
            ...
            <p className="mt-6 text-center text-xs leading-5 text-violet-300/50">
