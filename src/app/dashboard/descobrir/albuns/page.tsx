...
export default async function AlbumsPage({
  ...
  let report: Awaited<ReturnType<typeof getAlbumOpportunityReport>> | null = null;
  let loadFailed = false;
  try {
    report = await getAlbumOpportunityReport(session.user.id, { artistLimit: 5, top: 5 });
  } catch (error) {
    loadFailed = true;
    console.error("ALBUM-UI recommendation load failed", error);
  }
    ...
    <div className="space-y-5">
      ...
      <section className="product-panel p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          {report ? (
            <div className="flex gap-2 text-xs font-bold text-muted-inverse">
      ...
      {loadFailed || !report ? (
        <section className="product-panel p-6">
          <div className="status-warning flex items-start gap-3 rounded-2xl border px-4 py-4">
            ...
            <div>
              <p className="font-black">Não foi possível atualizar as recomendações agora.</p>
              <p className="mt-1 text-sm opacity-80">
                Nenhuma alteração foi feita. Tente novamente quando o Spotify estiver disponível.
              </p>
            </div>
          </div>
        </section>
      ...
      {report && report.providerMetrics.failures.length > 0 ? (
        <p className="px-1 text-xs text-muted-inverse">
          Algumas fontes não responderam ({report.providerMetrics.failures.length}); as sugestões exibidas foram calculadas apenas com dados válidos disponíveis.
        </p>
      ) : null}
    </div>
  );
